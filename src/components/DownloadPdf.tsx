...
export default function DownloadSection() {
  ...
  return (
    ...
    <section 
      className="w-full h-[450px] px-[145px] py-[65px] mx-auto flex flex-col justify-center items-center bg-[#F5F1E6]"
    >

      <div className=" flex flex-col items-center gap-[40px]">
        

        <h2 className={`font-crimson-text max-w-[808px] h-[87px] text-[22px] font-semibold leading-[100%] text-center text-[#162B75]`}>
        ...
        <div className="max-w-[1150px] h-[113px] bg-white px-[39px] py-[30px] flex items-center justify-between">
            
      
            <div className="flex items-center max-w-[1072px] h-[53px] gap-[15px]">
              {documents.map((doc, index) => (
                <a href={`/pdfFiles/${doc.src}`} download={doc.src} key={index} className="flex items-center gap-[5px] pr-[15px] border-r-2 border-[#162B75] last:border-none h-[53px]">
        ...
        <div           className=" w-[30px] h-[40px]"> 
          ...
          className='w-[30px] h-[40px]'
                   ...
                   <div className="flex flex-col gap-[2px] justify-center font-montserrat">
                      <span className="text-[14px] leading-[20px] not-italic text-[#000000] font-normal tracking-normal uppercase">{doc.sub}</span>
                      <div className="text-[#162B75] font-bold text-[14px] leading-[20px] uppercase whitespace-nowrap">
                        {doc.title}
                      </div>
                   </div>
                </a>
            ...
            <button onClick={handleDownloadZip} className="h-[46px] w-[192px] bg-[#162B75] flex items-center justify-center hover:bg-[#DCA447] transition-colors">
             <div className='h-[24px] max-w-[106px] text-[16px] leading-[24px] font-montserrat text-white font-normal  whitespace-nowrap' > Tải bộ tài liệu</div>
            </button>
        </div>

      
        <p className={`font-montserrat max-w-[808px] h-[48px] text-[16px] leading-[24px] text-center font-normal text-black`}>
